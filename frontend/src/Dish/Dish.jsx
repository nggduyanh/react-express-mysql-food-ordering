...
import { GetFoodRestaurant, GetSellerInfo } from "../routebackend";
...
export default function Dish() {
  ...
  useEffect(() => {
    fetch(GetFoodRestaurant)
      ...
      .then((data) => {
        console.log("sellerPageDish", seller);
        const filterDish = data.filter((dish) => {
          return dish.MaNguoiBan === seller.MaNguoiBan;
        });
  ...
  const handleRemove = async () => {
    const res = await axios.delete(GetFoodRestaurant);
    const findDish = data.find((dish) => dish.MaMonAn === filterDish.MaMonAn);
