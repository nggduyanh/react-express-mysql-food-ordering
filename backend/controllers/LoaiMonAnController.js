...
class LoaiMonAnController 
{
    ...
    {
        let obj = await LoaiMonAn.getById (req.params.id)
        if (!obj.success) return next (new Exception (obj.res,500))
    ...
    {
        let obj = await LoaiMonAn.add (req.body)
    ...
    {
        let obj = await LoaiMonAn.update (req.body)
        if (!obj.success) return next (new Exception (obj.res,400))
    ...
    {
        let obj = await LoaiMonAn.remove (req.body)
        if (!obj.success) return next (new Exception (obj.res,400))
...
module.exports = new LoaiMonAnController