...
async function CreatePayment (req,res){
    ...
    const embed_data = {
        ...
        // redirecturl: process.env.clientDomain+"/home"
        orderInfo: req.body 
    }
    ...
    try {
        const result= await axios.post(process.env.endpointCreate,null,{params: order})
...
async function CallBack(req,res){
    ...
    try {
        ...
        if(reqMac==mac){
            ...
            await DonHang.update({MaDonHang:JSON.parse(JSON.parse(dataStr).embed_data).orderInfo.MaDonHang,TrangThaiThanhToan: true})
    ...
    async function OrderStatus(trans_id,orderID){
        ...
        const data = reqData.app_id + "|" + reqData.app_trans_id + "|" + process.env.key1
